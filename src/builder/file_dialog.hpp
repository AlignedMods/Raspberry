...
GuiWindowFileDialogState InitGuiWindowFileDialog(const char *initPath)
{
    ...
    // Init window data
    state.windowBounds = (Rectangle){ GetScreenWidth()/2 - 440, GetScreenHeight()/2 - 310, 880, 620 };
...
void GuiWindowFileDialog(GuiWindowFileDialogState *state)
{
    if (state->windowActive)
    {
        ...
        int prevTextAlign = GuiGetStyle(DEFAULT, TEXT_ALIGNMENT);
...
# endif
        ...
        GuiSetStyle(DEFAULT, TEXT_ALIGNMENT, prevTextAlign);
