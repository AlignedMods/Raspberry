...
void Player::CheckCollisions() {
  ...
  for (auto rec : tiles) {
    // Only check for horizontal collisions if the player is on the ground (or
    // if that's your intended behavior)
    if (CheckCollisionRecs(
            {m_X, m_Y, m_Size, m_Size},
            {rec.GetX(), rec.GetY(), rec.GetSize(), rec.GetSize()})) {
      // Going right
...
void Player::Draw() { DrawRectangle(m_X, m_Y, m_Size, m_Size, RED); }
