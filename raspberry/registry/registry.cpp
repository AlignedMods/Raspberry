...
void s_Registry::RegisterArchive(const std::filesystem::path &archive) {
    Log(Log_Info, "Adding archive: %s", archive.string().c_str());

    if (!std::filesystem::exists(archive)) {
        Log(Log_Critical, "Archive does %s NOT exist!", archive.string().c_str());
    }
    ...
    Log(Log_Info, "Archive size: %llu bytes", contents.size());
    ...
    while (index < contents.size()) {
        ...
        Log(Log_Info, "Registered %s: %s (%d bytes)", type.c_str(), name.c_str(), size);
    }
    ...
    Log(Log_Info, "Finished registering archive %s!", archive.string().c_str());
}
...
void s_Registry::RegisterLevel(const std::string &name, const std::string &lvlData) {
    ...
    if (str != "RSP") {
        Log(Log_Warning, "Not a valid .lvl file!");
        return;
    }
    ...
    if (str != "_H_E_") {
        Log(Log_Warning, ".rsp File Header doesn't contain \"_H_E_\" at the correct spot!");
        return;
    }
    ...
    Log(Log_Info, "Successfully loaded level!");
